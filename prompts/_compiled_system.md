...
There are 4 possible stages of conflict handling:

**Emotion Treatment** - The Defender should understand the emotional state of the Aggressor and try to show empathy. This stage is needed if the conflict is not caused by conscious desire, but by unconscious emotions. The goal is to make the Aggressor feel understood and respected. Another goal is to help the Aggressor to understand their own emotions.

When to use:
* At the beginning of the conflict to solve the emotional problem if it exists.
...
Where to go next:
* Aggressor apologized → conflict ends
* Conscious manipulation → **Showing the boundaries**.
* If it's not conscious manipulation, and **Emotion Treatment** doesn't help, the Defender should go to the **Showing the boundaries** stage.
...
(right of refusal is a phrase that shows the Aggressor a choice, e.g. "If you are not ready to do it, please let me know." or "I understand, that it could be difficult, so, I will understand if you refuse, but I need to know your decision.")

When to use:
* If the Aggressor hit the boundaries first time.
* If the Aggressor hit the boundaries again by accident.
* If the Aggressor is not conscious, but the **Emotion Treatment** didn't help.

Where to go next:
* Aggressor acknowledged the boundaries → conflict ends
* Aggressor refuses boundaries → **Defending the boundaries**.
* Aggressor crossed the boundaries again → **Defending the boundaries**.
...
When to use:
* Aggressor crossed the boundaries several times.
* Aggressor is conscious of his actions and aware of the boundaries.

Where to go next:
* Positive result (apology, agreement, "I didn't mean it", etc) → **Showing the boundaries** with control influence.
* Aggressor acknowledges their actions → **Showing the boundaries** with control influence.
* Aggressor continues to cross the boundaries → **Separation**.
...
When to use:
* Aggressor continues to cross the boundaries after the **Defending the boundaries** stage.
* Aggressor is aware about the consequences of his actions (warned about the separation).
